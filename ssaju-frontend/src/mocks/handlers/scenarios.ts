...
export const errorScenarios = {
  /** 관운 분석 — 5초 타임아웃 */
  careerTimingTimeout: http.post(`${BASE_URL}/api/career/timing`, async () => {
    await delay(5500);
  ...
  /** 관운 분석 — 잘못된 날짜 형식 */
  invalidDateFormat: http.post(`${BASE_URL}/api/career/timing`, async () => {
    await delay(200);
    return HttpResponse.json(
  ...
  /** AI 컨설팅 — 20초 타임아웃 */
  consultationTimeout: http.post(`${BASE_URL}/api/career/consultation`, async () => {
    await delay(20500);
    return HttpResponse.json(
  ...
  /** 기업 궁합 — 기업 미발견 */
  companyNotFound: http.post(`${BASE_URL}/api/company/compatibility`, async () => {
    await delay(300);
    return HttpResponse.json(
  ...
  /** 네트워크 에러 시뮬레이션 */
  networkError: http.post(`${BASE_URL}/api/career/timing`, () => {
    return HttpResponse.error();
  }),

  /** 서버 에러 (500) */
  serverError: http.post(`${BASE_URL}/api/career/timing`, async () => {
    await delay(200);
    return HttpResponse.json(
