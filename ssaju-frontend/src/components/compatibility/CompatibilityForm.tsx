...
import type { RoleCategory } from '@/types/api';
...
interface CompatibilityFormProps {
  ...
  onSelectCompany: (company: DartCompany) => void;
  onClearCompany: () => void;
  onToggleDirectMode: (enabled: boolean) => void;
  onDirectInputChange: (value: string) => void;
  onRoleCategoryChange: (value: RoleCategory) => void;
  onRoleDetailNameChange: (value: string) => void;
  onSubmit: () => void;
}

/** 기업 궁합 분석 — 기업명/직군/세부 직무명 입력 폼 */
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {ROLE_CATEGORIES.map((r) => {
              ...
              return (
                <button
                  key={r.value}
                  onClick={() => onRoleCategoryChange(r.value)}
                  style={{
                    padding: '8px 16px',
                    borderRadius: 100,
                    background: active
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <div style={{ position: 'relative' }}>
            <input
              ...
              onChange={(e) => onRoleDetailNameChange(e.target.value)}
