...
/**
 * AI 커리어 컨설팅 — 스크롤형 상세 뷰 (마이페이지 기록 상세용)
 * FullPageConsultation의 모든 섹션을 세로 스크롤로 배치
 */
...
function SectionCard({
  ...
  return (
    <div
      style={{
        background: `radial-gradient(ellipse at 70% 10%, ${color}10 0%, transparent 55%)`,
...
function Label({ color, children }: { color: string; children: React.ReactNode }) {
  return (
    <p style={{ fontSize: 10, color, letterSpacing: '0.18em', textTransform: 'uppercase', fontWeight: 800, marginBottom: 10 }}>
      {children}
    </p>
  );
}
...
function Chip({ color, children }: { color: string; children: React.ReactNode }) {
  return (
    <span style={{ fontSize: 12, padding: '4px 12px', borderRadius: 999, background: `${color}14`, border: `1px solid ${color}35`, color }}>
      {children}
    </span>
  );
}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
      {items.map(({ label, value }) => (
        <div key={label} style={{ padding: '10px 12px', borderRadius: 10, background: `${color}07`, border: `1px solid ${color}18` }}>
  ...
  return (
    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'flex-start' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
        {tags.map((t, i) => (
          <span key={i} style={{ fontSize: 11, padding: '2px 9px', borderRadius: 999, background: `${tagColor}12`, border: `1px solid ${tagColor}28`, color: tagColor }}>
            {t}
          </span>
        ))}
      </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      >
        <div style={{ flex: 1, minWidth: 140 }}>
          ...
          <p
            ...
            style={{ fontSize: 'clamp(2.2rem, 7vw, 3rem)', color: '#fff', lineHeight: 1, letterSpacing: '-0.03em', textShadow: `0 0 40px ${color}55` }}
          >
            {period}
          </p>
        </div>
      ...
      {data.analysisSummary && (
        <div style={{ padding: '10px 14px', borderRadius: 10, background: `${color}0c`, border: `1px solid ${color}22` }}>
          <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.45)', lineHeight: 1.6 }}>{data.analysisSummary}</p>
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
        <div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {data.strengths.map((s, i) => <Chip key={i} color="#34d399">{s}</Chip>)}
          </div>
        </div>
        <div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
        ...
        >
          <span style={{ fontSize: 34, fontWeight: 900, color, fontFamily: 'serif' }}>{profile.dayMaster}</span>
        </div>
        <div>
            ...
            return (
              <div key={el} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div style={{ flex: 1, height: 7, borderRadius: 999, background: 'rgba(255,255,255,0.06)', overflow: 'hidden' }}>
                  <div style={{ width: `${pct}%`, height: '100%', background: `linear-gradient(90deg, ${c}, ${c}aa)`, borderRadius: 999 }} />
                </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
          {Object.entries(profile.tenGodDistribution).map(([god, val]) => (
            <div key={god} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 10, background: `${color}0e`, border: `1px solid ${color}2a` }}>
        ...
        <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {profile.keyTenGods.map((god, i) => (
            <span key={i} style={{ fontSize: 13, fontWeight: 900, padding: '4px 14px', borderRadius: 999, background: `${color}1a`, border: `1.5px solid ${color}55`, color, fontFamily: 'serif', boxShadow: `0 0 10px ${color}22` }}>
              {god}
            </span>
          ))}
        </div>
        {profile.tenGodCharacteristics && Object.keys(profile.tenGodCharacteristics).length > 0 && (
          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            {profile.keyTenGods.filter((god) => profile.tenGodCharacteristics![god]).map((god) => (
              <div key={god} style={{ padding: '12px 14px', borderRadius: 12, background: `${color}08`, border: `1px solid ${color}20` }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {industries.map((ind, i) => (
        <div
          key={i}
          style={{ padding: '20px 22px', borderRadius: 18, background: `${color}08`, border: `1px solid ${color}25`, position: 'relative', overflow: 'hidden' }}
        >
          <span style={{ position: 'absolute', top: 10, right: 16, fontSize: 44, fontWeight: 900, color: `${color}0a`, lineHeight: 1 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {ind.recommendedRoles.map((role, j) => (
              <span key={j} style={{ fontSize: 11, padding: '3px 11px', borderRadius: 999, background: `${color}14`, border: `1px solid ${color}33`, color }}>
                {role}
              </span>
            ))}
          </div>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {interviewTips.map((tip, i) => (
            <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'flex-start', padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
              <span style={{ flexShrink: 0, fontSize: 10, fontWeight: 900, color, width: 22, height: 22, borderRadius: 6, background: `${color}18`, border: `1px solid ${color}33`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {i + 1}
              </span>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {powerKeywords.keywords.map((kw: PowerKeywordItem, i: number) => (
            <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '12px 14px', borderRadius: 12, background: `${color}07`, border: `1px solid ${color}1a` }}>
              <span style={{ fontSize: 10, fontWeight: 900, color, flexShrink: 0, padding: '2px 8px', borderRadius: 6, background: `${color}1c`, border: `1px solid ${color}3a` }}>
                {kw.element}
              </span>
              <div style={{ flex: 1 }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        ...
        <p style={{ fontSize: 'clamp(0.95rem, 3vw, 1.2rem)', fontWeight: 800, color: '#fff', fontStyle: 'italic', lineHeight: 1.5, textShadow: `0 0 20px ${color}44` }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {[
          ...
          <div key={label} style={{ padding: '14px 16px', borderRadius: 14, background: `${color}08`, border: `1px solid ${color}1e` }}>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {powerKeywords.usageTips.map((tip, i) => (
            <span key={i} style={{ fontSize: 11, padding: '4px 12px', borderRadius: 999, background: `${color}0f`, border: `1px solid ${color}2a`, color: `${color}cc` }}>
              ✓ {tip}
            </span>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      {pivotPoints.map((pt: CareerPivotPoint, i: number) => {
        const s = PIVOT_STYLE[pt.type?.toUpperCase()] ?? PIVOT_STYLE.NORMAL;
        return (
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {Object.entries(months).map(([month, forecast]) => (
              <div key={month} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '10px 14px', borderRadius: 10, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
                <span style={{ fontSize: 12, fontWeight: 700, color, minWidth: 50, flexShrink: 0 }}>{month}월</span>
      ...
      <div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 16 }}>
          {warningMonths.map((m, i) => (
            <span key={i} style={{ fontSize: 12, fontWeight: 700, padding: '5px 13px', borderRadius: 999, background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.35)', color: '#f87171' }}>
              ⚠ {m}월
            </span>
          ))}
        </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {phases.map(({ label, phase, icon }, i) => (
        <div key={i} style={{ padding: '20px 22px', borderRadius: 18, background: `${color}09`, border: `1px solid ${color}22` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            <span style={{ fontSize: 18 }}>{icon}</span>
            <span style={{ fontSize: 10, fontWeight: 900, color, padding: '3px 10px', borderRadius: 6, background: `${color}1e`, border: `1px solid ${color}3a` }}>
              {label}
            </span>
      ...
      <div style={{ padding: '22px 24px', borderRadius: 20, background: `linear-gradient(135deg, ${color}16, ${color}08)`, border: `1.5px solid ${color}38`, boxShadow: `0 0 30px ${color}14` }}>
        <p style={{ fontSize: 10, color, letterSpacing: '0.16em', textTransform: 'uppercase', fontWeight: 700, marginBottom: 10 }}>✦ 궁극의 목표</p>
        <p style={{ fontSize: 20, fontWeight: 900, color: '#fff', marginBottom: 8, lineHeight: 1.3 }}>{longTermRoadmap.ultimateGoal}</p>
        <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.6)', lineHeight: 1.75 }}>{longTermRoadmap.goalDescription}</p>
      </div>
    </div>
  );
}
...
interface ConsultationScrollDetailProps {
  data: ConsultationData;
}

export function ConsultationScrollDetail({ data }: ConsultationScrollDetailProps) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <SectionCard index={0} label={SECTIONS[0].label} color={SECTIONS[0].color}>
        <CoreSection data={data} color={SECTIONS[0].color} />
      </SectionCard>

      <SectionCard index={1} label={SECTIONS[1].label} color={SECTIONS[1].color}>
        <SajuSection profile={data.sajuProfile} color={SECTIONS[1].color} />
      </SectionCard>

      <SectionCard index={2} label={SECTIONS[2].label} color={SECTIONS[2].color}>
        <IndustrySection industries={data.industries} color={SECTIONS[2].color} />
      </SectionCard>

      <SectionCard index={3} label={SECTIONS[3].label} color={SECTIONS[3].color}>
        <InterviewSection data={data} color={SECTIONS[3].color} />
      </SectionCard>

      <SectionCard index={4} label={SECTIONS[4].label} color={SECTIONS[4].color}>
        <BrandingSection data={data} color={SECTIONS[4].color} />
      </SectionCard>

      {data.careerTimeline?.pivotPoints?.length > 0 && (
        <SectionCard index={5} label={SECTIONS[5].label} color={SECTIONS[5].color}>
          <PivotSection timeline={data.careerTimeline} color={SECTIONS[5].color} />
        </SectionCard>
      )}

      {data.careerTimeline && (
        <SectionCard index={6} label={SECTIONS[6].label} color={SECTIONS[6].color}>
          <ForecastSection timeline={data.careerTimeline} color={SECTIONS[6].color} />
        </SectionCard>
      )}

      <SectionCard index={7} label={SECTIONS[7].label} color={SECTIONS[7].color}>
        <RoadmapSection data={data} color={SECTIONS[7].color} />
      </SectionCard>

      <SectionCard index={8} label={SECTIONS[8].label} color={SECTIONS[8].color}>
        <WorkStyleSection data={data} color={SECTIONS[8].color} />
      </SectionCard>

      <SectionCard index={9} label={SECTIONS[9].label} color={SECTIONS[9].color}>
        <MentalSection data={data} color={SECTIONS[9].color} />
      </SectionCard>

      <SectionCard index={10} label={SECTIONS[10].label} color={SECTIONS[10].color}>
        <WealthSection data={data} color={SECTIONS[10].color} />
      </SectionCard>
    </div>
  );
}
